    ...
    <AppLayout>
        <div>
            <div class="flex justify-between items-center mb-4">
                <input v-model="search" @input="filterUsers" type="text" placeholder="Search system users..."
                    class="border rounded px-4 py-2 w-full max-w-sm" />
                <button @click="showCreateModal = true"
                    class="ml-4 bg-blue-600 text-white px-4 py-2 rounded-md  hover:bg-blue-700">Add System User</button>
            </div>
            ...
            <SystemUserFormModal v-if="showCreateModal" :roles="roles" :user="editingUser" @close="closeModal"
                @saved="refresh" />
        </div>
    </AppLayout>
</template>
